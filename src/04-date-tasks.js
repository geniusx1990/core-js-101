...
function timeSpanToString(startDate, endDate) {
  ...
  const h = `${end.getHours() - start.getHours()}`;
  const m = `${end.getMinutes() - start.getMinutes()}`;
  const s = `${end.getSeconds() - start.getSeconds()}`;
...
function angleBetweenClockHands(date) {
  ...
  const hourAngle = Math.abs(0.5 * (60 * hours + minutes));
